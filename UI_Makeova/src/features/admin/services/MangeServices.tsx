...
import { BadgeCheck, PauseCircle, Sparkles, type LucideIcon } from 'lucide-react'
  ...
  return (
    <AdminLayout>
      ...
      {!loading && (
        <div className="grid grid-cols-3 gap-3 mb-5">
          {[
            { label: 'Total Services', value: stats.total, icon: Sparkles, color: '#C49A7A', bg: '#FDF0EB' },
            { label: 'Active Services', value: stats.active, icon: BadgeCheck, color: '#4CAF50', bg: '#E8F5E9' },
            { label: 'Inactive Services', value: stats.inactive, icon: PauseCircle, color: '#aaa', bg: '#F5F5F5' },
          ].map(({ label, value, icon: Icon, color, bg }: { label: string; value: number; icon: LucideIcon; color: string; bg: string }) => (
      ...
      <div className="bg-white rounded-2xl border border-[#F0DDD5] shadow-[0_2px_16px_rgba(196,154,122,0.08)] overflow-hidden">
        ...
        {!loading && !error && services.length === 0 && (
          <div className="py-14 text-center">
            <div className="w-12 h-12 rounded-full bg-[#FDF0EB] flex items-center justify-center mx-auto mb-3 text-[#C49A7A]">
        ...
        {!loading && !error && (
          <div className="table-scroll-area">
            ...
            <div>
              <span className="text-[12px] font-bold text-[#C49A7A] font-serif">â‚¹{s.price.toLocaleString()}</span>
            </div>
